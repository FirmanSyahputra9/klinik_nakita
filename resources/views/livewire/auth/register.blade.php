<x-layouts.auth>
    <div class="flex flex-col gap-6">
        ...
        <div class="max-h-[70vh] overflow-y-auto pr-2 p-4">
            <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-6">
                ...
                <flux:input name="birth_date" :label="__('Tanggal Lahir')" value="{{ old('birth_date') }}"
                    type="date" required autofocus autocomplete="tanggal_lahir" :placeholder="__('Tanggal Lahir')"
                    min="1900-01-01" max="{{ now()->toDateString() }}" />

                <!-- Alamat -->
                <flux:input name="alamat" :label="__('Alamat')" value="{{ old('alamat') }}" type="text" required
                    autofocus autocomplete="alamat" :placeholder="__('Alamat')" />
