<x-layouts.app :title="__('Dashboard')">
    <main class="flex flex-col lg:flex-row gap-6 p-4 md:p-6">
        ...
        <section class="flex-1 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <!-- Card 1 -->
                <article class="card bg-white rounded-xl shadow-md p-5 flex flex-col gap-3 border border-gray-100 hover:shadow-lg transition">
                    ...
                    <div>
                        <p class="card text-lg font-semibold text-gray-800">Cek Jadwal</p>
                ...
                <article class="card bg-white rounded-xl shadow-md p-5 flex flex-col gap-3 border border-gray-100 hover:shadow-lg transition">
                    ...
                    <div>
                        <p class="card text-lg font-semibold text-gray-800">Riwayat Medis</p>
                ...
                <article class="card bg-white rounded-xl shadow-md p-5 flex flex-col gap-3 border border-gray-100 hover:shadow-lg transition">
                    ...
                    <div>
                        <p class="card text-lg font-semibold text-gray-800">Hasil Laboratorium</p>
            ...
            <section class="card bg-white rounded-xl shadow p-6">
                <h3 class="card font-semibold text-lg mb-4 text-gray-800">Nomor Registrasi</h3>
                @foreach ($antrian_registrasi ?? [] as $ar)
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="bg-pink-50 rounded-lg p-4 text-center shadow">
                        <p class="card text-xs text-gray-500">Nomor Antrian Registrasi</p>
                        <p class="card text-2xl font-bold text-pink-600">{{ $ar->appointment_code }}</p>
                        <p class="card text-xs text-gray-400 mt-1">{{ $ar->create_at }}</p>
                    </div>
                    <div class="bg-blue-50 rounded-lg p-4 text-center shadow">
                        <p class="card text-xs text-gray-500">Dokter Pilihan</p>
                        <p class="card text-2xl font-bold text-blue-600">{{ $ar->nama_dokter }}</p>
                    </div>
                    <div class="bg-green-50 rounded-lg p-4 text-center shadow">
                        <p class="card text-xs text-gray-500">Antrian pada tanggal {{ $ar->tanggal_kunjungan }}</p>
                        <p class="card text-2xl font-bold text-green-600">{{ $ar->antrian_registrasi }}</p>
                    </div>
                </div>
                @endforeach
            </section>
        ...
        <!-- RIGHT SIDEBAR -->
        <aside class="card w-full lg:w-72 bg-white rounded-3xl p-6 shadow space-y-6">

            <!-- Profile -->
            <div class="card flex flex-col items-center">
                <div class="w-24 h-24 rounded-full bg-gray-300 mb-3"></div>
                <p class="card font-semibold text-gray-800 text-lg">{{ $user->pasien->name }}</p>
                <p class="card font-semibold text-gray-700 text-sm">{{ $user->pasien->no_rm }}</p>
            </div>
            <!-- Health Stats -->
            <div class="card grid grid-cols-2 text-center text-sm gap-4">
                <div class="card p-2 bg-white rounded-lg shadow">
                    <p class="card font-medium text-gray-800">Gol. Darah</p>
                    <p class="card text-gray-800 font-semibold">{{ $user->pasien->gol_darah }}</p>
                </div>
                <div class="card p-2 bg-white rounded-lg shadow">
                    <p class="card font-medium text-gray-800">Usia</p>
                    <p class="card font-semibold text-gray-800">{{ $user->pasien->umur }}</p>
                </div>
            </div>

            <!-- Health Notes -->
            <a href="{{ route('riwayat.index') }}"
                class="card w-full bg-slate-50 hover:bg-sky-200 transition py-2 rounded-xl flex items-center justify-center text-sm font-medium text-gray-800 shadow">
                <span>Riwayat</span>
    ...
    <!-- Resep Obat -->
    <section class="card bg-white rounded-xl shadow overflow-hidden mx-4 md:mx-6 mb-6">

        <!-- Header -->
        <header class="card bg-white p-4 space-y-1">
            <h3 class="card font-bold text-lg text-gray-900">Resep Obat</h3>
        ...
        <!-- Content -->
        <article class="card bg-gray-50 rounded-lg p-4">
            <div class="card flex items-start gap-3 mb-3">
                ...
                <div>
                    <h5 class="card font-semibold text-gray-900">Doxycycline 100 mg</h5>
                    <p class="card text-sm text-gray-600">10 Kapsul • Dosis: 100 mg</p>
                </div>
            </div>

            <dl class="card space-y-2 text-sm">
                <div class="flex justify-between">
                    <dt class="text-gray-600 card">Frekuensi:</dt>
                    <dd class="font-medium text-gray-900 card">1 x 1 Tablet</dd>
                </div>

                <div class="flex justify-between">
                    <dt class="text-gray-600 card">Waktu Konsumsi:</dt>
                    <dd class="font-medium text-gray-900 card">Pagi setelah makan</dd>
                </div>

                <div class="flex justify-between">
                    <dt class="text-gray-600 card">Harga:</dt>
                    <dd class="font-medium text-gray-900 card">Rp 25.000</dd>
                </div>
            </dl>
        </article>
    </section>
